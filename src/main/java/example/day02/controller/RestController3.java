...
//@Component // Bean 등록 (싱글톤 대용)
//@Controller // Bean 등록 (싱글톤 대용) + HTTP 통신 기능                              = 사용처 : view(화면) 반환
@RestController // Bean 등록 (싱글톤 대용) + HTTP 통신 기능 + ResponseBody 포함      = 사용처 : 값(json) 반환
...
public class RestController3 {
    ...
    // 6. http://localhost:8080/day02/task11?name=유재석
    @PostMapping("/task11")
    ...
    // GET/DELETE -> 쿼리스트링 -> @RequestParam/@ModelAtrribute
    // POST/PUT -> 쿼리스트링 + BODY본문 -> @RequestBody
    // 즉2] URL 상의 매개변수 노출이 가리기 위한 BODY(본문) 사용하자
    //      개인정보/패스워드/민감한정보들 은 POST/PUT BODY(본문) 사용하자
